...
We will use same prior setting (described in Section~\ref{sec:ex-prior})
and combine it with three different data scenarios,
where we observe failure times in accordance with prior expectations in the first case (Section~\ref{sec:ex-case1}),
...
In each case, we assume that C2, C3, P2 and P3 fail, only varying the failure times,
and study the effect of learning about these failures on the component level.
...
The prior assumptions, which we can imagine to be determined by an expert,
...
(Note that we number component failure times by order, not by component number in the system layout.)
...
\end{figure}
%
Having observed the system only until $t = 2$, our data is not very informative,
...
mainly because observing $\vec{t}^\text{P} = (0.25, 0.5, 2^+, 2^+)$ for P is not as extreme
...
our data is much more informative,
...
after having seen these late failures, we expect type C components to fail much later.
...
as we know that the system has survived until $\tnow = 8, 2, 12$ in each of the three cases.
...
the right panel instead on the scale of prospective time,
indicating how far in the future periods are for which estimation of system reliability is most uncertain.
We see that posterior imprecision is indeed considerably lower in case~1,
where we had observed failures more or less like expected.
On a prospective timescale, we see that periods of heightened uncertainty are closer to the present for the posteriors,
