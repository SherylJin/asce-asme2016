...
(\citeNP{1994:berger}) and `imprecise probability theory' (\citeNP{1991:walley,itip}),
...
Prior-data conflict has attracted some attention in the literature (see, e.g., \citeNP{2006:evans}),
...
The approach used in this paper was introduced by \citeNP{Walter2009a} (see also \citeNP{diss} \S 3.1.4).
...
standard `precise' Bayesian approach. For example, \citeN{2008:bousquet} proposed a method for diagnostics 
of agreement or disagreement between the prior and data, based on a ratio of Kullback-Leibler divergences.
\citeN{2012:ohagan-pericchi} propose the use of heavy-tailed distributions which, in case of conflict 
...
There are $n_k$ components of type $k$ ($k=1,\ldots,K$) in the systems, their
...
We will first explain %in Section~\ref{sec:lambdawithcens}
...
as introduced above. %in Section~\ref{sec:modforsurpr}.
...
%and can also be found in the github repository \texttt{http://www.github.com/geeeero/asce-asme2016}.
...
%(Section~\ref{sec:ex-case1}),
surprisingly early failures in the second case, and %(Section~\ref{sec:ex-case2}), and
